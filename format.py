###################################################格式化报文，去除多余空行########################################################
#
#python .\format.py --input_file "D:\Downloads\1.xlsx" --row 2 --column 16 --output_file "D:\Downloads\2.xlsx" --output_column 23
#
#################################################################################################################################
...
class excel_sender:
    ...
    def __init__(self,input_file,row,column,output_file,output_column, log_func=None, finish_callback=None):
        ...
        #v1.8，输出文件，如果不存在复制源文件，如果存在打开，可以中断之后再执行不会覆盖原来的数据。
            ...
            for row in ws.iter_rows(min_row=start_row, max_row=max_row,  min_col=col, max_col=col,values_only=True):
                if any(cell is not None for cell in row):  # 检查行是否包含非空单元格
                    tmp_row=row[0]
                    a=self.format(tmp_row)
                    # print(a)
                    ws1.cell(row=start_row, column=col1, value=a)
                start_row += 1
...
def main():
    ...
    test=excel_sender(input_file,row,column,output_file,output_column)
    print("转换中，请稍候......")
    test.read_excel()
    # print("检测完成！")
